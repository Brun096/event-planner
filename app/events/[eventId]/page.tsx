...
export default async function EventPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const eventResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/events/${eventId}`, {
  ...
  const notGoingRSVPs = event.rsvps.filter((rsvp) => rsvp.status == 'NOT_GOING');
        ...
        <div className='grid md:grid-cols-2 gap-8'>
          <div className='space-y-4'>
            <div className='flex items-center'>
              ...
              <div>
                <p className='font-medium text-foreground'>{format(new Date(event.date), 'EEEE, MMMM do, yyyy')}</p>
                <p className='text-muted'>{format(new Date(event.date), 'h:m a')}</p>
              </div>
            </div>
            ...
            <div className='flex items-center'>
              ...
              <span className='text-foreground'>Organized by {event.user.name || event.user.email}</span>
            </div>
          ...
          {!isPast && event.isPublic && session && <RSVPButtons eventId={event.id} currentRSVP={currentRSVP} />}
