...
export default function RSVPButtons({ eventId, currentRSVP }: RSVPButtonsProps) {
  ...
  function getButtonClass(status: RSVPStatus) {
    ...
    switch (status) {
      case 'GOING':
        return `${baseClass} ${
          isActive ? 'bg-green-600 text-white' : 'bg-green-600/20 text-green-400 hover:bg-green-600/30'
        }`;
      case 'NOT_GOING':
        return `${baseClass} ${isActive ? 'bg-red-600 text-white' : 'bg-red-600/20 text-red-400 hover:bg-red-600/30'}`;
      case 'MAYBE':
        return `${baseClass} ${
          isActive ? 'bg-yellow-600 text-white' : 'bg-yellow-600/20 text-yellow-400 hover:bg-yellow-600/30'
        }`;
      default:
        return baseClass;
    }
  ...
  return (
    <div className='space-y-4'>
      <h3 className='text-lg font-semibold text-foreground'>RSVP to this event</h3>
      <div className='flex flex-wrap gap-3'>
        <button disabled={isLoading} className={getButtonClass('GOING')} onClick={() => handleRSVP('GOING')}>
          Going
        </button>
        <button disabled={isLoading} className={getButtonClass('MAYBE')} onClick={() => handleRSVP('MAYBE')}>
          Maybe
        </button>
        <button disabled={isLoading} className={getButtonClass('NOT_GOING')} onClick={() => handleRSVP('NOT_GOING')}>
          Not Going
        </button>
      </div>
    </div>
